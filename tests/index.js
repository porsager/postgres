...
t('Ensure rows count is cleared before performing new query on a connection', async() => {
  ...
  await sql`create table some_other_table(text,int)`
  await sql`insert into some_other_table values('a', 100)`
  await sql`insert into some_other_table values('b', 200)`
  await sql`insert into some_other_table values('c', 300)`
  await sql`insert into some_other_table values('d', 400)`
  await sql`insert into some_other_table values('e', 500)`
  await sql`insert into some_other_table values('f', 600)`

  const rows_count_before_error = await sql`select count(*) from some_other_table`
  await sql`select * from table_causing_error where 1000/age > 0`.catch(e => e)
  const rows_count_after_error = await sql`select count(*) from some_other_table`

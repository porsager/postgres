...
function Postgres(a, b) {
  ...
  function partial(xs) {
    const o = Object.create(notPromise)
    o.partial = xs
    o.args = Array.from(arguments).slice(1)
    return o
  }
  ...
  function parse(query, xs, args = [], xargs = [], types = []) {
    let str = xs[0]
    let arg
  ...
  function parseArg(str, arg, xargs, types) {
    if (arg && arg.P === notPromise.P) {
      if (arg.array) return parseArray(arg.array, xargs, types)
      if (arg.partial) return parsePartial(arg.partial, arg.args, xargs, types)
      if (arg.skip) return ''
      return parseHelper(str, arg, xargs, types)
    }

    return parseValue(arg, xargs, types)
  }

  function parsePartial(xs, args, xargs, types) {
    const { str } = parse({}, xs, args, xargs, types)
    return str
  }

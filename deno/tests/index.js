...
const exec = (x, xs) => new Promise((r, e) => cp.spawn(x, xs).on('close', x => x ? e(x) : r()))
const login = {
...
t('Fail with proper error on no host', async() => {
  ...
  return [true, err.code === 'ECONNREFUSED' || err.name === 'ConnectionRefused']
})
...
t('sql file throws', async() =>
  [true, (await sql.file('./selectomondo.sql').catch(x => x.code === 'ENOENT' || x.name === 'NotFound'))]
)
...
t('Connection errors are caught using begin()', {
  ...
  return [true, error.code === 'ENOTFOUND' || error.message.includes('failed to lookup address')]
})
...
t('Big result', { timeout: 2000 }, async() => {

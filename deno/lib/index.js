  ...
  function writable(promise, query) {
    ...
    return query.readable = new Stream.Duplex({
      ...
      destroy(error, callback) {
        callback(error)
        error && query.writable.push({ error })
      },
      final(callback) {
        if (error)
...
function parseOptions(a, b) {
  ...
  return Object.assign({
    ...
    port            : host.split(',').map(x => parseInt(x.split(':')[1] || port)),
    path            : o.path || host.indexOf('/') > -1 && host + '/.s.PGSQL.' + port,
    database        : o.database || o.db || (url.pathname || '').slice(1) || env.PGDATABASE || user,
    user            : user,
    pass            : o.pass || o.password || url.password || env.PGPASSWORD || '',
    max             : o.max || url.searchParams.get('max') || 10,
    types           : o.types || {},
    ssl             : o.ssl || parseSSL(url.searchParams.get('sslmode') || url.searchParams.get('ssl')) || false,
    idle_timeout    : o.idle_timeout || url.searchParams.get('idle_timeout') || env.PGIDLE_TIMEOUT || warn(o.timeout),
    connect_timeout : o.connect_timeout || url.searchParams.get('connect_timeout') || env.PGCONNECT_TIMEOUT || 30,
    ...
    target_session_attrs: o.target_session_attrs
...
function parseUrl(x) {
  if (typeof x !== 'string')
    return { url: { searchParams: { get: () => undefined } } }
  ...
  return {
    url: new URL(x.replace(host, host.split(',')[0])),
